  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${CREAM};padding:28px 12px;">
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#fffdf9;border:1px solid #ece2cf;border-radius:16px;overflow:hidden;">
        <tr><td style="background:linear-gradient(135deg,#f3f7f2,#2c1454);padding:28px 32px;text-align:center;">
          <div style="color:${GOLD};font-size:13px;letter-spacing:3px;text-transform:uppercase;">✦ Awaken Again ✦</div>
          <div style="color:#e5d5b5;font-family:Georgia,serif;font-size:21px;margin-top:6px;">Amber's Alchemy Apothecary</div>
        </td></tr>
        <tr><td style="padding:34px 32px;color:${INK};font-family:Georgia,'Times New Roman',serif;font-size:16px;line-height:1.65;">
          ${innerHtml}
        </td></tr>
        <tr><td style="background:#f4e8d0;padding:22px 32px;text-align:center;color:#c8b79a;font-family:Arial,sans-serif;font-size:12px;line-height:1.6;">
          ${footerHtml || ''}
...
const SUBSCRIPTION_BLURB_HTML = `
  <div style="margin:24px 0;padding:18px 20px;background:#1e1148;border-left:3px solid ${GREEN};border-radius:8px;">
...
function weeklyPromo(unsubscribeUrl, weekIndex = 0) {
  ...
  const footer = `You're receiving this gentle weekly note because you ordered from or joined Amber's Alchemy Apothecary.
    <br><a href="${esc(unsubscribeUrl)}" style="color:#c8b79a;text-decoration:underline;">Unsubscribe from weekly notes</a>`;
...
const GRIMOIRE_BENEFITS_HTML = `
  <div style="margin:22px 0;padding:18px 20px;background:#1e1148;border-left:3px solid ${GREEN};border-radius:8px;">
...
function grimoireMonthly(opts = {}) {
  ...
  const footer = `You're receiving this as an active Grimoire subscriber.
    <br><a href="${esc(unsubscribeUrl)}" style="color:#c8b79a;text-decoration:underline;">Manage your subscription</a>`;
